...
function checkemailsignup() {
	return JS_CheckEmail($('#email').val(), oktip($('#email')),
			errortipsignup($('#email')), 'register');
}
function checkpasswordsignup() {
	return JS_CheckPassword($('#passwd').val(), oktip($('#passwd')),
			errortipsignup($('#passwd')));
}
function checknickname() {
	return JS_CheckNickname($('#nickname').val(), oktip($('#nickname')),
			errortipsignup($('#nickname')));
}
function checkcode() {
...
function checkdoublepassword() {
	if (checkpasswordsignup() && $('#passwd').val() == $('#dpasswd').val()) {
		...
		if ($('#dpasswd').val() == '') {
			errortipsignup($('#dpasswd'))('Please input confirm password');
			return false;
		} else {
			errortipsignup($('#dpasswd'))('Two passwords are not identical');
			return false;
		}
	}
}
...
function oktip(obj) {
	return function() {
		obj.parents('.iput').next().html(' ').addClass('loright').addClass(
				'mtl15').removeClass('lowrong').removeClass('mll');
	}
}
...
(function($) {
	$.fn.emailMatch = function(settings) {
		var defaultSettings = {
			emailTip : "",
			aEmail : [ "qq.com", "gmail.com", "126.com", "163.com",
					"hotmail.com", "yahoo.com", "yahoo.com.cn", "live.com",
					"sohu.com", "sina.com" ], // 邮件数组
			wrapLayer : "body",
		...
		/* 合并默认参数和用户自定义参数 */
		settings = settings ? $.extend(defaultSettings, settings)
				: defaultSettings;
		return this
				.each(function() {
					var elem = $(this), t = 0, l = 0, w = elem.outerWidth(), h = elem
							.outerHeight(), selectVal = sMail = inputVal = "", arrayNum = 0, isIndex = -1;
							...
							function() {
								...
								if ($.trim(elem.val()) != "") {
									...
									if (isIndex >= 0) {
										...
										inputVal = inputVal.substring(0,
												isIndex);
										if (sMail != "") {
											arrayNum = parseInt(!position(
													settings.aEmail, sMail) ? 0
													: position(settings.aEmail,
															sMail));
										}
									}
									...
									createMailList(mailWrap, inputVal, sMail,
											settings.aEmail, arrayNum);
								}
								;
							}).blur(function() {
						if (elem.val() == '') {
					...
					elem
							.keyup(function(e) {
								...
								switch (eKey) {
								case 8: // backspace
									$(mailWrap).find("li.current").removeClass(
											"current");
									break;
								case 9: // tab 按键
									return;
								...
								case 13: { // 回车
									...
									if ($(mailWrap).find("li.current").text() != ""
											&& keyIndex >= 0) {
										enterVal(mailWrap, elem);
									}
									hideList($(mailWrap));
								...
								default: {
									...
									if (keyIndex >= 0) {
										...
										if (suffix != ''
												&& settings.emailRemember) { // 过滤数组
											for ( var i = 0; i < settings.aEmail.length; i++) {
												if (settings.aEmail[i]
														.indexOf(suffix) == 0) {
													suffixArray
															.push(settings.aEmail[i]);
													suffixState = false;
												}
											}
										}
										if (suffix != ''
												&& !settings.emailRemember) { // 当前高亮
											// 选项
											for ( var i = 0; i < settings.aEmail.length; i++) {
												if (settings.aEmail[i]
														.indexOf(suffix) == 0) {
									...
									suffixArray = suffixArray.length > 0 ? suffixArray
											: settings.aEmail;
									if (inputVal == "" && suffix == "") {
										hideList($(mailWrap));
										arrayNum = 0;
										createMailList(mailWrap, inputVal,
												suffix, settings.aEmail,
												arrayNum);
									} else {
					...
					$(mailWrap).find("li:not('.first')").live(
							'mouseover',
							function() {
	...
	function createMailList(oWrap, sVal, suffix, aEail, nK) { // 创建 候选列表
		...
		nK = parseInt(suffix != '' && State && !position(aEail, suffix) ? 0
				: nK);

		if (suffix != '' && State) {
			if (nK == 0) {
				mailList += '<li class="current"><span>' + sVal + '</span>@'
						+ suffix + '</li>';
			} else {
		...
		if ($.isArray(aEail)) {
			$.each(aEail, function(i) {
				if (State && suffix != '') {
					if (i == (nK - 1)) {
						mailList += '<li class="current"><span>' + sVal
								+ '</span>@' + aEail[i] + '</li>';
					} else {
						mailList += '<li><span>' + sVal + '</span>@' + aEail[i]
								+ '</li>';
					}
				} else {
					if (i == nK) {
